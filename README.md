...
<img src="./image/image1.png" alt="Alt text" width="800">
 ...
 > **CIDR Block** : As we have 4 subnet and we have total of 256 IP's allocated for this VPC in CIDR block we will allocate each subnet with 64 IP's
 >
 >CIDR blocks will be :
 >
 > 1.1.1.0/26 , 1.1.1.64/26 , 1.1.1.128/26 and 1.1.1.192/26
 > 
 > **For each availability zone create 1 private and 1 public subnet** - A subnet becomes only public or private by attaching it with the NAT gateway or Internet gateway

 <img src="./image/image3.png" alt="Alt text" width="800">


#### **III - Create Nat Gateway and Internet Gateway to make the subnet public and private :**
#### **Creating IGW:**
...
Now, Same way create for Private-Route-Table.
...
> Now, the subnets became truly Public and Private
 ...
 #### **V - Checking the Flow of create VPC**
...
We will deploy the Application Server in private-subnet-1, which means it cannot be accessed directly from the internet.
...
<img src="./b_app.png" alt="Alt text" width="800">
...
#### **Create Application Server (EC2 - Ubuntu)**
 ...
 >We only allow bastion host to make SSH connection to the application host by setting the inbound rule of application server.
...
Now that the Application Server is set up and securely accessible via the Bastion Host.
...
>4. **pip3 install --break-system-packages flask flask-mysql-connector**
...
The Application Server is now up and running, and it successfully communicates with the Database Server (RDS).
...
To regulate the traffic between various webservers we need implement Application Load Balancer.
